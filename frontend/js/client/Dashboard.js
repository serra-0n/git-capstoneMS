...
const uploadedDocumentsCount = document.getElementById("uploadedDocumentsCount");

/* Current reservation */

const currentReservationContent = document.getElementById("currentReservationContent");

const currentReservationEmptyState = document.getElementById("currentReservationEmptyState");

const currentReservationReference = document.getElementById("currentReservationReference");

const currentReservationStatus = document.getElementById("currentReservationStatus");

const currentReservationResort = document.getElementById("currentReservationResort");

const currentReservationAccommodation = document.getElementById("currentReservationAccommodation");

const currentReservationCheckIn = document.getElementById("currentReservationCheckIn");

const currentReservationCheckOut = document.getElementById("currentReservationCheckOut");

const viewReservationStatusButton = document.getElementById("viewReservationStatusButton");

/* START PAGE Dashboard.js is loaded at the bottom of the HTML, after Lucide, so the DOM and Lucide library are already ready. */
...
async function loadDashboardFromApi() {
    try {
        ...
        dashboardState.client = normalized.client;

        dashboardState.summary = normalized.summary;

        dashboardState.current_reservation = normalized.current_reservation;

        renderDashboard();
    } catch (error) {
        console.error("Dashboard loading error:", error);

        dashboardState.client = null;
        ...
        dashboardState.current_reservation = null;
...
function normalizeDashboardData(data) {
    ...
    const client = dashboard.client
        ? {
              id: dashboard.client.id ?? dashboard.client.client_id ?? null,

              name: dashboard.client.name || dashboard.client.full_name || "",
          }
        : null;
    ...
    const summary = {
        active_reservations: toSafeNumber(sourceSummary.active_reservations),

        payment_status: sourceSummary.payment_status || "",

        uploaded_documents: toSafeNumber(sourceSummary.uploaded_documents),
    };
    ...
    return {
        ...
        current_reservation: normalizeReservation(dashboard.current_reservation),
    };
}
...
function renderClient() {
    const clientName = dashboardState.client?.name || "Client";

    setText(clientDisplayName, clientName);

    if (dashboardWelcomeTitle) {
        dashboardWelcomeTitle.textContent = `Welcome, ${clientName}`;
    }
}
...
function renderSummary() {
    setText(
        ...
        dashboardState.summary.active_reservations,
    );

    setText(
        dashboardPaymentStatus,

        dashboardState.summary.payment_status,
    );

    setText(
        uploadedDocumentsCount,

        dashboardState.summary.uploaded_documents,
    );
}
...
function renderCurrentReservation() {
    const reservation = dashboardState.current_reservation;
    ...
    renderReservationStatus(reservation.reservation_status);

    if (viewReservationStatusButton && reservation.id) {
        viewReservationStatusButton.href = `ReservationStatus.html?id=${encodeURIComponent(
            reservation.id,
        )}`;
    }
}
...
function renderReservationStatus(status) {
    ...
    currentReservationStatus.textContent = status || "—";

    currentReservationStatus.classList.remove(
        "status-success",
        "status-warning",
        "status-danger",
        "status-info",
    ...
    currentReservationStatus.classList.add(getStatusClass(status));
}
...
function showCurrentReservationEmptyState() {
    if (currentReservationContent) {
        currentReservationContent.hidden = true;
    }

    if (currentReservationEmptyState) {
        currentReservationEmptyState.hidden = false;
    }
}

function hideCurrentReservationEmptyState() {
    if (currentReservationContent) {
        currentReservationContent.hidden = false;
    }

    if (currentReservationEmptyState) {
        currentReservationEmptyState.hidden = true;
    }
}
...
function initializeIcons() {
    if (typeof window.lucide === "undefined") {
        console.error("Lucide library did not load.");

        return;
    }

    if (typeof window.lucide.createIcons !== "function") {
        console.error("lucide.createIcons() is unavailable.");
