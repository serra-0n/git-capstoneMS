...
const selectedDocumentFile = document.getElementById("selectedDocumentFile");

const selectedDocumentFileName = document.getElementById("selectedDocumentFileName");

const selectedDocumentFileSize = document.getElementById("selectedDocumentFileSize");

const removeDocumentFileButton = document.getElementById("removeDocumentFileButton");

const documentFormMessage = document.getElementById("documentFormMessage");

const submitDocumentButton = document.getElementById("submitDocumentButton");

/* Uploaded documents */

const uploadedDocumentList = document.getElementById("uploadedDocumentList");

const uploadedDocumentsEmptyState = document.getElementById("uploadedDocumentsEmptyState");

/* Detail */

const documentDetailCard = document.getElementById("documentDetailCard");

const documentDetailType = document.getElementById("documentDetailType");

const documentDetailReservation = document.getElementById("documentDetailReservation");

const documentDetailFileName = document.getElementById("documentDetailFileName");

const documentDetailStatus = document.getElementById("documentDetailStatus");

/* INITIALIZATION */

document.addEventListener("DOMContentLoaded", initializeUploadDocumentsPage);
...
async function loadDocumentDataFromDatabase() {
    try {
        await Promise.all([loadClientFromApi(), loadReservationsFromApi(), loadDocumentsFromApi()]);
        ...
        console.error("Unable to load document information:", error);

        showDocumentMessage("Unable to load document information.", "error");
    }
}
...
async function loadClientFromApi() {
    ...
    documentState.client = normalizeClient(data);
}
...
async function loadReservationsFromApi() {
    ...
    documentState.reservations = normalizeReservations(data);
}
...
async function loadDocumentsFromApi() {
    ...
    documentState.documents = normalizeDocuments(data);
}
...
function normalizeReservations(data) {
    const records = Array.isArray(data) ? data : data?.reservations;

    if (!Array.isArray(records)) {
        return [];
    }

    return records.map(normalizeReservation).filter(Boolean);
}
...
function normalizeDocuments(data) {
    ...
    return records.map(normalizeDocument).filter(Boolean);
}
...
function normalizeDocument(document) {
    ...
    return {
        ...
        file_name: document.file_name || document.original_file_name || "",

        verification_status:
            document.verification_status || document.status || "Pending Verification",
    };
}
...
function renderClient() {
    ...
    clientDisplayName.textContent = documentState.client?.name || "Client";
}
...
function renderReservations() {
    ...
    placeholder.textContent = "Select reservation";

    reservationId.appendChild(placeholder);

    documentState.reservations.forEach((reservation) => {
        const option = document.createElement("option");

        option.value = String(reservation.id);

        option.textContent = createReservationOptionText(reservation);

        reservationId.appendChild(option);
    });
}

/* RESERVATION OPTION TEXT */

function createReservationOptionText(reservation) {
    const parts = [];

    if (reservation.reference) {
        parts.push(reservation.reference);
    }

    if (reservation.resort_name) {
        parts.push(reservation.resort_name);
    }

    return parts.join(" - ");
}

/* PRESELECT RESERVATION FROM URL Example: UploadDocuments.html?reservation=105 */
...
function applyReservationFromUrl() {
    ...
    const requestedReservationId = parameters.get("reservation");

    if (!requestedReservationId) {
        return;
    }

    const exists = documentState.reservations.some(
        (reservation) => String(reservation.id) === String(requestedReservationId),
    );

    if (exists) {
        reservationId.value = requestedReservationId;
    }
}
...
function renderDocuments() {
    ...
    if (!Array.isArray(documentState.documents) || documentState.documents.length === 0) {
    ...
    documentState.documents.forEach((documentRecord) => {
        const item = createDocumentItem(documentRecord);

        uploadedDocumentList.appendChild(item);
    });
...
function createDocumentItem(documentRecord) {
    ...
    button.className = "uploaded-document-item";

    button.dataset.documentId = String(documentRecord.id ?? "");

    /* Icon */

    const iconWrapper = document.createElement("span");

    iconWrapper.className = "uploaded-document-item-icon";

    const icon = document.createElement("i");

    icon.setAttribute("data-lucide", getDocumentIcon(documentRecord.document_type));
    ...
    content.className = "uploaded-document-item-content";

    const heading = document.createElement("div");

    heading.className = "uploaded-document-item-heading";

    const title = document.createElement("strong");

    title.textContent = getDocumentTypeLabel(documentRecord.document_type);

    const status = document.createElement("span");

    status.className = `status-badge document-status ${getStatusClass(
        documentRecord.verification_status,
    )}`;

    status.textContent = documentRecord.verification_status || "—";
    ...
    meta.className = "uploaded-document-item-meta";

    if (documentRecord.reservation_reference) {
        ...
        reservationText.textContent = documentRecord.reservation_reference;
    ...
    if (documentRecord.file_name) {
        ...
        fileText.textContent = documentRecord.file_name;

        fileMeta.append(fileIcon, fileText);
...
function selectDocument(documentId) {
    const documentRecord = documentState.documents.find(
    ...
    documentState.selectedDocument = documentRecord;

    updateSelectedDocumentItem(documentId);

    renderDocumentDetails(documentRecord);
}

/* SELECTED DOCUMENT STYLE */

function updateSelectedDocumentItem(documentId) {
    const documentItems = document.querySelectorAll(".uploaded-document-item");

    documentItems.forEach((item) => {
        const selected = String(item.dataset.documentId) === String(documentId);
...
function renderDocumentDetails(documentRecord) {
    ...
    setText(documentDetailType, getDocumentTypeLabel(documentRecord.document_type));

    setText(documentDetailReservation, documentRecord.reservation_reference);

    setText(documentDetailFileName, documentRecord.file_name);

    renderDetailStatus(documentRecord.verification_status);
...
function renderDetailStatus(status) {
    ...
    documentDetailStatus.textContent = status || "—";
    ...
    documentDetailStatus.classList.add(getStatusClass(status));
}

/* STATUS PRESENTATION These frontend mappings are presentation-only. Backend values eventually become the source of truth. */
...
function handleFileSelection() {
    ...
    documentState.selectedFile = file;
...
function renderSelectedFile(file) {
    ...
    selectedDocumentFile.hidden = false;

    setText(selectedDocumentFileName, file.name);

    setText(selectedDocumentFileSize, formatFileSize(file.size));
...
function hideSelectedFile() {
    documentState.selectedFile = null;

    if (selectedDocumentFile) {
        selectedDocumentFile.hidden = true;
    }
...
function initializeRemoveFileButton() {
    ...
    removeDocumentFileButton.addEventListener("click", removeSelectedFile);
}
...
async function handleDocumentUpload(event) {
    ...
    const validationMessage = validateDocumentUpload();

    if (validationMessage) {
        showDocumentMessage(validationMessage, "error");
...
function createDocumentFormData() {
    ...
    formData.append("reservation_id", reservationId?.value || "");

    formData.append("document_type", documentType?.value || "");

    if (documentState.selectedFile) {
        formData.append("document_file", documentState.selectedFile);
    }
...
async function uploadDocumentToApi(formData) {
    ...
    try {
        ...
        const uploadedDocument = normalizeDocument(data?.document || data);

        if (uploadedDocument) {
            documentState.documents.unshift(uploadedDocument);
        ...
        showDocumentMessage(data?.message || "Document uploaded successfully.", "success");
    } catch (error) {
        console.error("Document upload error:", error);

        showDocumentMessage(error.message || "Unable to upload document.", "error");
    } finally {
        setSubmittingState(false);
    }
}
...
function setSubmittingState(submitting) {
    ...
    submitDocumentButton.disabled = submitting;

    submitDocumentButton.classList.toggle("is-loading", submitting);

    const buttonText = submitDocumentButton.querySelector("span");

    if (buttonText) {
        buttonText.textContent = submitting ? "Uploading..." : "Upload Document";
    }
}

/* FORM MESSAGE */

function showDocumentMessage(message, type = "info") {
    ...
    documentFormMessage.append(icon, messageText);

    documentFormMessage.classList.remove("success", "error", "info");

    documentFormMessage.classList.add(type);

    documentFormMessage.hidden = false;
...
function clearDocumentMessage() {
    ...
    documentFormMessage.hidden = true;
}
...
function showDocumentsEmptyState() {
    if (uploadedDocumentList) {
        uploadedDocumentList.hidden = true;
    }

    if (uploadedDocumentsEmptyState) {
        uploadedDocumentsEmptyState.hidden = false;
    }
...
function hideDocumentsEmptyState() {
    if (uploadedDocumentList) {
        uploadedDocumentList.hidden = false;
    }

    if (uploadedDocumentsEmptyState) {
        uploadedDocumentsEmptyState.hidden = true;
    }
}
...
/* LUCIDE ICONS Must be called again whenever icons are inserted dynamically through JavaScript. */
