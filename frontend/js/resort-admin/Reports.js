...
const exportButtons = document.querySelectorAll(".report-action");
...
function isDateInRange(dateValue, start, end) {
    const recordDate = new Date(dateValue + "T00:00:00");
    ...
    if (end) {
        const endDateValue = new Date(end + "T23:59:59");
...
function getFilteredReservations() {
    ...
    return reservationRecords.filter((reservation) => isDateInRange(reservation.date, start, end));
}
...
function getFilteredPayments() {
    ...
    return paymentRecords.filter((payment) => isDateInRange(payment.date, start, end));
}
...
function getFilteredVerificationRecords() {
    ...
    return verificationRecords.filter((record) => isDateInRange(record.date, start, end));
}

/* CALCULATE RESERVATION REPORT */

function calculateReservationReport() {
    const records = getFilteredReservations();
...
function calculateOccupancyReport() {
    const total = accommodationRecords.length;

    const available = accommodationRecords.filter(
        (accommodation) => accommodation.status === "Available",
    ).length;

    const reserved = accommodationRecords.filter(
        (accommodation) => accommodation.status === "Reserved",
    ).length;

    const occupied = accommodationRecords.filter(
        (accommodation) => accommodation.status === "Occupied",
    ).length;

    const occupiedAndReserved = occupied + reserved;

    const occupancyPercentage = total > 0 ? Math.round((occupiedAndReserved / total) * 100) : 0;
...
function calculatePaymentReport() {
    const records = getFilteredPayments();
    ...
    const pending = records
        .filter((payment) => payment.status === "Pending Verification")
        .reduce((sum, payment) => sum + payment.amount, 0);
...
function calculateTransactionReport() {
    ...
    const completed = records.filter((payment) => payment.status === "Paid").length;

    const pending = records.filter((payment) => payment.status === "Pending Verification").length;

    const unpaid = records.filter((payment) => payment.status === "Outstanding").length;
...
function calculateVerificationReport() {
    const records = getFilteredVerificationRecords();
...
function updateSummaryCards() {
    const reservationReport = calculateReservationReport();

    const occupancyReport = calculateOccupancyReport();

    const paymentReport = calculatePaymentReport();

    const transactionReport = calculateTransactionReport();
    ...
    if (reservationTotal) {
        reservationTotal.textContent = formatNumber(reservationReport.total);
    }
    ...
    const occupancyTotal = document.getElementById("occupancyTotal");

    if (occupancyTotal) {
        occupancyTotal.textContent = `${occupancyReport.occupancyPercentage}%`;
    }
    ...
    if (paymentTotal) {
        paymentTotal.textContent = formatCurrency(paymentReport.total);
    }
    ...
    if (transactionTotal) {
        transactionTotal.textContent = formatNumber(transactionReport.total);
    }
}
...
function updateReservationReport() {
    const report = calculateReservationReport();

    const section = document.querySelector('[data-report-section="reservation"]');

    if (!section) {
        return;
    }

    const items = section.querySelectorAll(".report-item strong");

    if (items.length >= 4) {
        items[0].textContent = formatNumber(report.total);

        items[1].textContent = formatNumber(report.confirmed);

        items[2].textContent = formatNumber(report.pending);

        items[3].textContent = formatNumber(report.cancelled);
    }
}
...
function updateOccupancyReport() {
    const report = calculateOccupancyReport();

    const section = document.querySelector('[data-report-section="occupancy"]');
    ...
    if (items.length >= 4) {
        ...
        items[1].textContent = formatNumber(report.available);

        items[2].textContent = formatNumber(report.reserved);

        items[3].textContent = formatNumber(report.occupied);
    }
}
...
function updatePaymentReport() {
    ...
    const section = document.querySelector('[data-report-section="payment"]');
    ...
    if (items.length >= 4) {
        items[0].textContent = formatCurrency(report.total);

        items[1].textContent = formatCurrency(report.paid);

        items[2].textContent = formatCurrency(report.pending);

        items[3].textContent = formatCurrency(report.outstanding);
    }
}
...
function updateTransactionReport() {
    ...
    const section = document.querySelector('[data-report-section="transaction"]');
    ...
    if (items.length >= 4) {
        ...
        items[1].textContent = formatNumber(report.completed);

        items[2].textContent = formatNumber(report.pending);

        items[3].textContent = formatNumber(report.unpaid);
    }
}
...
function updateVerificationReport() {
    const report = calculateVerificationReport();

    const section = document.querySelector('[data-report-section="verification"]');
    ...
    if (items.length >= 4) {
        items[0].textContent = formatNumber(report.submitted);

        items[1].textContent = formatNumber(report.verified);

        items[2].textContent = formatNumber(report.pending);

        items[3].textContent = formatNumber(report.rejected);
    }
}
...
function showSelectedReport() {
    const selectedType = reportType ? reportType.value : "all";
    ...
    reportSections.forEach((section) => {
        const sectionType = section.dataset.reportSection;
...
function applyReportFilter() {
    ...
    if (selectedType !== "all") {
        targetSection = document.querySelector(`[data-report-section="${selectedType}"]`);
    } else {
        targetSection = document.querySelector(".report-section");
    }
...
function generateReport() {
    ...
    let reportName = "All Reports";

    switch (selectedType) {
        case "reservation":
            reportName = "Reservation Summary";
            break;

        case "occupancy":
            reportName = "Occupancy Monitoring";
            break;

        case "payment":
            reportName = "Payment Records";
            break;
        ...
        case "verification":
            reportName = "Document Verification";
            break;
    }
    ...
    let periodText = "All available dates";
...
if (applyFilterButton) {
    applyFilterButton.addEventListener("click", applyReportFilter);
}
