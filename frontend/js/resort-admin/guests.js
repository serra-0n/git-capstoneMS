/* =========================================================
   ...
   NOTE:
   The guest records currently displayed in Guests.html
   are SAMPLE DATA ONLY.

   These records can later be replaced with database/API
   data without changing the page structure.
   ========================================================= */
...
const guestStatusFilter =
  document.getElementById("guestStatusFilter");

const clearGuestFilter =
  document.getElementById("clearGuestFilter");

const refreshGuestsButton =
  document.getElementById("refreshGuestsButton");

const guestTableBody =
  document.getElementById("guestTableBody");

const emptyGuestState =
  document.getElementById("emptyGuestState");
...
const currentGuests =
  document.getElementById("currentGuests");
...
function filterGuests() {
  ...
  const searchValue =
    guestSearch
      ? normalizeText(guestSearch.value)
      : "";

  const statusValue =
    guestStatusFilter
      ? guestStatusFilter.value
      : "all";
  ...
  rows.forEach(row => {
    ...
    const matchesSearch =
      ...
      reservation.includes(searchValue);
...
function updateRecordCount(
  ...
  if (tableResultText) {
    ...
    if (visibleCount === 0) {
      ...
      tableResultText.textContent =
        `Showing ${totalCount} guest records`;

    } else {

      tableResultText.textContent =
        `Showing ${visibleCount} of ${totalCount} guest records`;
...
function updateSummaryCounts() {
  ...
  if (guestsWithReservations) {
    ...
    guestsWithReservations.textContent =
      withReservations;
...
function viewGuestRecord(row) {
  ...
  const guestName =
    row.dataset.guest || "Guest";


  const reservation =
    row.dataset.reservation || "No reservation";
  ...
  const guestId =
    guestIdElement
      ? guestIdElement.textContent.trim()
      : "Guest ID unavailable";
  ...
  if (contactElement) {
    ...
    const contactText =
      contactElement.querySelector("span");

    if (contactText) {

      contact =
        contactText.textContent.trim();
...
if (guestStatusFilter) {
  ...
  guestStatusFilter.addEventListener(
    "change",
    filterGuests
  );
...
if (clearGuestFilter) {
  ...
  clearGuestFilter.addEventListener(
    "click",
    clearFilters
  );
...
if (refreshGuestsButton) {
  ...
  refreshGuestsButton.addEventListener(
    "click",
    refreshGuestRecords
  );
